...
final class DefinitionPermissionCheckerTest extends TestCase
{
    ...
    /**
     * Проверяет, что RoleDefinition и checker принимают BackedEnum.
     */
    #[Test]
    public function allowsGrantedBackedEnumPermissions(): void
    {
        $provider = new ArrayRoleDefinitionProvider(
            roles: [
                RoleDefinition::named('support')
                    ->allow(TestPermissionEnum::PostsRead),
            ],
            permissions: [TestPermissionEnum::PostsRead],
        );
        ...
        self::assertTrue($checker->can($user, TestPermissionEnum::PostsRead));
        self::assertFalse($checker->can($user, TestPermissionEnum::AdminAccess));
    }
}
