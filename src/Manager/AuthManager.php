...
use PhpSoftBox\Auth\Authorization\PermissionDecisionCheckerInterface;
...
final class AuthManager
{
    ...
    public function decide(mixed $user, string|BackedEnum $permission, mixed $subject = null): AccessDecision
    {
        ...
        if ($this->permissions === null) {
            return AccessDecision::deny('Permission checker is not configured.');
        }

        if ($this->permissions instanceof PermissionDecisionCheckerInterface) {
            return $this->permissions->decide($user, $permission, $subject);
        }
