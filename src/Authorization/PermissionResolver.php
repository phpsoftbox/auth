...
final class PermissionResolver
{
     ...
     * @return list<string>
     */
    public function resolve(PermissionGrant|string|BackedEnum $grant): array
    {
        if (is_string($grant)) {
            ...
            $name = PermissionName::normalize($grant);

            return $name === '' ? [] : [$name];
        }

        if ($grant instanceof BackedEnum) {
            $name = PermissionName::normalize($grant);
    ...
    /**
     * @param list<BackedEnum|string> $actions
     * @return list<string>
     */
    private function resolveResource(string $resource, array $actions, string $scope): array
    {
        ...
        foreach ($actions as $action) {
            $actionValue = $action instanceof BackedEnum ? (string) $action->value : (string) $action;
