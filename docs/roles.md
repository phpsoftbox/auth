...
Имя permission можно передавать строкой или `BackedEnum`:

```php
enum UserPermissionName: string
{
    case Read   = 'users.base.read';
    case Update = 'users.base.update';
}

$auth->can($user, UserPermissionName::Update);
```
...
Если `PermissionStore` поддерживает `TransactionalStoreInterface`, `sync()` выполняется в транзакции.
...
В качестве пользователя можно передать объект, реализующий `UserInterface`,
...
`$user` должен реализовать `UserInterface`, а объект — `OwnableInterface`:
