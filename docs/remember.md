...
$service = new MultiGuardRememberService(
    ...
    guards: [
        ...
        'site' => new RememberGuardConfig(
            store: new DatabaseRememberTokenStore($connections),
            cookies: new RememberCookieManager($cookieQueue, new RememberCookieConfig(
                name: 'remember_site',
                path: '/',
            )),
            extractor: new RememberTokenExtractor('remember_site'),
            users: $siteUsers,
            guard: $siteGuard,
            metadata: ['scope' => 'site'],
        ),
...
$intended = new IntendedUrlStore(
    session: $session,
    key: 'site.auth.intended',
    excludePaths: ['/site/login'],
    excludePrefixes: ['/site/logout', '/site/password'],
);

$intended->remember($request);

// После успешной авторизации:
$redirectTo = $intended->pull('/site');
```
...
`auth.intended` для центральной зоны и `site.auth.intended` для site-зоны.
