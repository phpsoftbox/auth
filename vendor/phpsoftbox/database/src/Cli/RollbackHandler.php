...
final class RollbackHandler implements HandlerInterface
{
    public function __construct(
        private readonly ConnectionManagerInterface $connections,
        private readonly ?MigrationsConfig $config = null,
    ...
    {
        if ($this->config === null) {
            $runner->io()->writeln('MigrationsConfig не сконфигурирован.', 'error');
        ...
        if (!is_int($steps) || $steps < 1) {
            $runner->io()->writeln('Некорректное значение steps.', 'error');
            return Response::FAILURE;
        }
        ...
        foreach ($paths as $path) {
            ...
            foreach ($loader->load($path, recursive: true) as $item) {
        ...
        try {
            $rolledBack = $runnerService->rollback($plan, $steps);
        } catch (Throwable $exception) {
            $runner->io()->writeln('Ошибка отката: ' . $exception->getMessage(), 'error');
            return Response::FAILURE;
        }

        $runner->io()->writeln('Откатано миграций: ' . count($rolledBack), 'success');
        foreach ($rolledBack as $id) {
